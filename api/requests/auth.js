...
import thinbus from 'thinbus-srp/client.js';
...
const SRP6JavascriptClientSession = thinbus(rfc5054.N_base10, rfc5054.g_base10, rfc5054.k_base16);
  ...
  */
    ...
    export const logInRequest = (formValues) => {
      ...
      return challengeRequest(formValues.id).then(challengeResponse => {
        ...
        // Do the actual login
        return Api.request().post('/login', values).then(res => {
          return res;
        }).catch(err => {
          return err;
        })
   ...
   */
    ...
    export const signUpRequest = (data) => {
      ...
      const password_salt = srpClient.generateRandomSalt();
      const password_verifier = srpClient.generateVerifier(password_salt, email, password);
